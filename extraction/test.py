...
def test_semantics():
  ...
  print("Semantics", termcolor.colored(f"Passed: {passed}", 'green'), termcolor.colored(f"Wrong: {wrong}", 'yellow'),termcolor.colored(f"Failed: {failed}", 'red'), sep='\t')
  ...
  with open(os.path.join(TEMP_FOLDER, f"SEMANTICS_wrong.csv"), "w") as wrong_log:
    wrong_details = pd.DataFrame({"index": wrong_fragments_indices, "fragment": wrong_fragments})
...
if __name__ == "__main__":
  ...
  # test_rule("rel", "active voice preposition", [nlp_patterns.active_voice_preposition], nlp_patterns.process_active_voice_preposition)
  # test_rule("rel", "noun with", [nlp_patterns.noun_with], nlp_patterns.process_noun_with)
  # print()
  # print("DETAILS")
